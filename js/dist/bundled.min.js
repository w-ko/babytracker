...
const getChildren = () => db.children.toArray(), getChild = (childId) => db.children.get(childId), createChild = (child) => {
  db.children.add(child);
}, updateChild = (childId, ...props) => {
  db.children.update(childId, ...props);
}, deleteChild = (childId) => {
  db.transaction("rw", db.children, db.timeline, () => {
    db.children.delete(childId), db.timeline.where("childId").equals(childId).delete();
  }).catch((error) => {
    console.error(error);
  });
}, getTimelineByChildId = (childId) => db.timeline.sortBy("start").where("childId").equals(childId).toArray(), getTimelineById = (timelineEntryId) => db.timeline.get(timelineEntryId), createTimelineEntry = (timelineEntry) => db.timeline.add(timelineEntry), updateTimelineEntry = (timelineEntryId, ...props) => db.timeline.update(timelineEntryId, ...props), deleteTimelineEntry = (timelineEntryId) => db.timeline.delete(timelineEntryId);
